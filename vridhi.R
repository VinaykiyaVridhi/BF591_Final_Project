...
ui <- fluidPage(
    ...
    p("Exploring RNA-Seq data from post-mortem BA9 brain tissue for Huntington's Disease and neurologically normal individuals."),
    mainPanel(
        tabsetPanel(
            ...
            tabPanel("Counts",
                     ...
                     sidebarPanel(fileInput("counts_data","Load counts data",accept = ".csv"),
                                  ...
                                  selectInput("x", "Select PC for x axis", 
                                              choices = c('PC1','PC2','PC3','PC4','PC5','PC6','PC7','PC8','PC9','PC10'), selected = 1),
                                  selectInput("y", "Select PC for y axis", 
            ...
            tabPanel("Differential Expression",
                     ...
                     sidebarPanel(fileInput("DE_data", "Load differential expression data", accept = ".csv"),
                                  radioButtons("x_axis", "Choose the parameter for the x-axis", 
                                               c("baseMean","HD.mean","Control.mean","log2FoldChange","lfcSE","stat","pvalue","padj"), selected = "log2FoldChange"),
                                  radioButtons("y_axis", "Choose the parameter for the y-axis", 
                                  ...
                                  sliderInput("slider", min = -25, max = 0,
                                              label = "Select the magnitude of the p adjusted",value = -15, step = 5),
                                  actionButton("go", "Go")
                     ),
                     mainPanel(
                         tabsetPanel(
            ...
            tabPanel("Gene Expression",
                     ...
                     sidebarPanel(fileInput("sample_data2", "Load sample data", accept = ".csv"),
                                  ...
                                  selectInput("categorical_value", label = ("Choose category to plot the data"), 
                                              choices = c('Control vs. HD Samples' = 1, 'Age at Death' = 2)),
...
server <- function(input, output, session)
{
    ...
    scatter_cv<- function(filtered)
    {
        ...
        p <- ggplot(df,aes(median,variance_cv)) + geom_point(aes(color = factor(Filtered)),alpha = 0.5) + 
            ...
            scale_color_manual(values = c('Filtered Out' = "blue", "UnFiltered" = "grey"))+
    ...
    #To create volacano plot
    volcano_plot <- function(dataf, x_axis, y_axis, slider, color1, color2) 
    {
        p <- ggplot(dataf, aes(x = !!sym(x_axis),
                               y = -log10(!!sym(y_axis)))) +
            geom_point(aes(color = !!sym(y_axis)<1*10^(as.numeric(slider)))) +
            scale_color_manual(values = c(color1, color2)) +
            theme(legend.position = "right") +
            theme_bw() +
            labs(color = paste0(y_axis, " <1 x 10^", slider))
        return(p)
    }
    ...
    output$vol_plot <- renderPlot({req(input$DE_data)
        df <- load_data_difexp()
    ...
    output$filter_table <- renderTable({req(input$DE_data)
        df <- load_data_difexp()
        p <- draw_table(df,input$slider)
        return(p)})
    ...
    control_affected<- function(sample,norm,gene_id){
        dataf<-subset(norm,gene==gene_id)
        ...
        h_col <- h$sample_id
        c_counts <- dataf %>% select(all_of(c_col)) %>% unlist() %>% list() %>%lapply(function(x) {x[x!=0]})
        h_counts <- dataf %>% select(all_of(h_col)) %>% unlist() %>% list() %>%lapply(function(x) {x[x!=0]})
        t_h<-data.frame(h_counts,letter = "H")
        t_c<- data.frame(c_counts,letter = "C")
        colnames(t_h) = c("counts","group")
        colnames(t_c) = c("counts","group")
        control_affected_df <- rbind(t_h,t_c)
        return(control_affected_df)
    }
    ...
    output$plot4 <- renderPlot({req(input$counts_data2)
        ...
        if(input$categorical_value ==1){
            data<- control_affected(df_samples,df_counts,gene_name)
        }
        else{
